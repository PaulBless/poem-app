    ...
    */
    ...
    public function registerAuth(Request $request)
    {
        try {
            ...
            if ($result) {
                return (new ResponseHelper(false, "User Already Exists", 400))->get();
            }

            $password = Hash::make($request->password);
            $result = $this->user->registerAuth($request->name, $request->email, $password);
            if ($result) {
                return (new ResponseHelper(true, 'registration successful', 200))->get();
            }

            return (new ResponseHelper(false, 'error while registering user', 400))->get();
